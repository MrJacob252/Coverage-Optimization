'''
Capacitated Location Set Covering Problem - Equal Assignment
From: R. L. Church, Alan Murray; Location Covering Models History, Applications and Advancements
ISBN: 978-3-319-99846-6
'''
from gamspy import Container, Set, Parameter, Variable, Equation, Sum, Model, Sense, Alias
import pandas as pd
import numpy as np
...
def clscp_ea(customer_locations: NDArray[Any],
    ...
    for i, row in enumerate(i_space):
        for j, col in enumerate(j_space):
            d_ij[str(row), str(col)] = 1 if distance_matrix[i][j] <= S.toValue() else 0
    ...
    # Subsets # TODO: Fix this section or rewrite it
    N_i = Alias(
        m,
        name="N_i",
        alias_with=I
    ...
    for i, row in enumerate(i_space):
        for j, col in enumerate(j_space):
            N_i[str(row)] = 1 if distance_matrix[i][j] <= S.toValue() else 0 # type: ignore
    ...
    for i, row in enumerate(i_space):
        for j, col in enumerate(j_space):
            Psi_j[str(col)] = 1 if d_ij[str(row), str(col)] == 1 else 0 # type: ignore
    # Variables
    ...
    y = Variable(
        ...
        name="y",
        domain=[I, K],
        description="1 if demand is covered by exactly k facilities",
        type="binary"
    )
    ...
    EQ1 = Equation(
        ...
        domain=[I],
        name="EQ1",
        description="Cover balance"
    )
    EQ2 = Equation(
        m,
        domain=[J],
        name="EQ2",
        description="Cover Unique"
    )
    EQ3 = Equation(
        m,
        domain=[I],
    ...
    capacities = pd.DataFrame(y.toList(), columns=["I", "K", "value"])
...
def __test():
    ...
    save_path = pathlib.Path("./src/models/tmp/clscp_ea_test")
    clscp_ea(c_loc, c_dem, s_loc, s_cap, distance_matrix, max_range, save_path)
